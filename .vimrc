...
"
"
"2020/04/22 Koyama（Vimが好きになる本の設定を参照）
"

"  Vim のバッファやレジスタ内などで使用する文字コードを設定
set  encoding=utf-8

" スクリプト ( この場合は vimrc) で使われている文字コードを宣言
"  Vim がスクリプトを処理するとき、 scriptencoding で指定した文字コードから
"  encoding で指定した文字コードに変換される
scriptencoding  utf-8

" 既存のファイルを開くとき、 Vim が使用する文字コードを判定する順番
" 先頭から順に試される
...
" 検索時、大字を指定した場合大文字と小文字を区別する
set  smartcase

 " 文字を検索するたびに、その時点でパターンマッチしたテキストをハイライト
 set  incsearch
" 展開するスペースの個数
set  tabstop=4
...
" Chrome で Javascript を実行結果を確認
let mapleader =','
nnoremap \ ,
nnoremap <Leader>cr :!start C:\Program Files\Google\Chrome\Application\chrome.exe file://%:p
